...
- Frontend Firebase SDK is initialized once in `JS/firebaseConfig.js`.
- Shared authenticated API calls are centralized in `JS/api.js`.
- Backend Firebase Admin uses environment variables from `backend/.env` and a sample file is included as `backend/.env.example`.
- Update `HTML/login.html` and `HTML/admin.html` script tags to `type="module"` so the shared Firebase config can be imported cleanly.


## Local backend startup

Frontend pages can open on `http://127.0.0.1:5500`, but the API must also be running on port `5000`.
...
Or use the helper files:

- Windows: `backend/run-dev.bat`
- Mac/Linux: `backend/run-dev.sh`

### If backend is on another machine

Use **Backend Settings** on the login page and enter the full backend URL, for example:
...
This project can now be deployed as one Node web service. See `DEPLOYMENT.md`.
